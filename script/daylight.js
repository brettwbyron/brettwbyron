...
// Daylight viz svg
var sunViz = d3.select('#sunlight_viz')
    .append('svg')
...
// Main sleep line graph variables
var sleepData;
// var sleepData = d3.csv.parse('data.csv', function(d) {
// 	return {
// 		item: d
// 	};
// });
sleepData = d3.csv("data.csv")
    .row(function (d) {
        return d;
    })
...
// Sleep min/max
var minTime = d3.min(sleepData, function (d) {
    return +d.Time;
});
var maxTime = d3.max(sleepData, function (d) {
    return +d.Time;
});
var minEff = d3.min(sleepData, function (d) {
    return +d.Efficiency;
});
var maxEff = d3.max(sleepData, function (d) {
    return +d.Efficiency;
});

var svgT = d3.select("#sleep_viz")
    .append("svg")
    .attr("width", w + padding * 2)
    .attr("height", hSleep + padding * 2);
...
//    var TlineFunction = d3.svg.line().data(sleepData).enter()
// 	                       .xScale(function(d, i) { return xScale(i);})
// 	                       .TyScale(function(d) { return TyScale(+d.Sleep_Time/60);})
// 	                       .interpolate("bundle");

// d3.csv("Data.csv", function(data) {		
// 	// svgT.append('g')
// 	// 	.attr('class', 'x axis')
// 	// 	.attr('transform', 'translate(0,' + hSleep + ')')
// 	// 	.call(xAxis);
...
// 	x.domain([1, 365]);
// 	y.domain(d3.extent(data, function(d) { return +d.Sleep_Time;}));

// 	svgT.selectAll('path')
//               .data(data)
//               .enter()
//               .append("path")
//               .attr("d", TlineFunction(data));
